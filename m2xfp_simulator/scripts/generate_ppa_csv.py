...
RTL_RESULT = ROOT.parent / "rtl_area_power" / "result"
...
def make_row(pmax, pmin, area_report, power_report, normalize=1):
    area = parse_area(RTL_RESULT / area_report) / normalize
    dynamic, leakage = parse_power(RTL_RESULT / power_report)
    return {
...
def main():
    default_rows = [
        make_row(
            8,
            4,
            "baselines/ant_olive/ant_pe_fusion_area_report.txt",
            "baselines/ant_olive/ant_pe_fusion_power_report.txt",
        ),
        make_row(
            8,
            2,
            "baselines/microscopiq/pe_microscopiq_area_report.txt",
            "baselines/microscopiq/pe_microscopiq_power_report.txt",
        ),
        make_row(
            4,
            4,
            "asplos26_28nm/pe_tile_v/area_28nm_report.txt",
            "asplos26_28nm/pe_tile_v/power_28nm_report.txt",
            normalize=8,
        ),
    ]
    ...
    mant_rows = [
        make_row(
            8,
            2,
            "baselines/mant/pe88_withFusion_withShift_area_report.txt",
            "baselines/mant/pe88_withFusion_withShift_power_report.txt",
        )
    ]

    write_csv(PPA_DIR / "systolic_array_synth.csv", default_rows)
