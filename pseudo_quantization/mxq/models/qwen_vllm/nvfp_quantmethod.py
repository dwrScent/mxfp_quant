...
from mxq.quantize.qmodule_nvfp import NVFP_Linear


class FakeQuantLinearMethod(UnquantizedLinearMethod):
    """
    NVFP fake-quantization linear method.
    Reuses the exact quantization pipeline from NVFP_Linear.
    """
    ...
    def apply(
        ...
        # 1. 第一次调用时，把普通 Linear → NVFP_Linear
        if not hasattr(layer, "_nvfp_linear"):
            layer._nvfp_linear = NVFP_Linear.from_linear(
                linear=layer,
                w_bit=self.w_bit,
                a_bit=self.a_bit,
        ...
        # 2. NVFP_Linear.forward  内部已经做了权重/输入量化
        out = nvfp_linear(x).to(torch.bfloat16)
...
class NVFPQuantConfig(QuantizationConfig):
    ...
    Replaces the old per-token dynamic fake-quant with NVFP.
    """
